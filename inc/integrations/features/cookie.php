...
function gtemplate_init_cookie_manager($gCore) {
    try {
        ...
        if (!$cookie) {
            gtemplate_track_error('gTemplate: CookieManager not available from gCore');
            return null;
        }
        ...
        gtemplate_track_error('gTemplate: CookieManager initialized for GDPR compliance');
        return $cookie;

    } catch (\Throwable $e) {
        gtemplate_track_error('gTemplate: CookieManager init error: ' . $e->getMessage());
        return null;
    }
}
...
function gtemplate_has_cookie_consent(string $category): bool {
    ...
    try {
        ...
        gtemplate_track_error('gTemplate: Cookie consent check error: ' . $e->getMessage());
    }
...
function gtemplate_update_cookie_consent(array $preferences): bool {
    ...
    try {
        ...
        gtemplate_track_error('gTemplate: Cookie consent update error: ' . $e->getMessage());
    }
...
add_action('rest_api_init', function() {
    // Commit 1.11.b: the POST endpoint mutates server
    // state (CookieManager + ValKey via gtemplate_update_cookie_consent),
    // so the previous `__return_true` permission callback let any
    // cross-origin `fetch(..., {credentials:'include'})` write user
    // cookie preferences. Replace with `gtemplate_rest_verify_nonce`
    // which checks `X-WP-Nonce` against the standard `wp_rest`
    // action — the WP REST API's canonical CSRF gate.
    ...
    // GET stays public — read-only, no state mutation.
    register_rest_route(gtemplate_get_rest_namespace(), '/cookie-consent', [
        'methods' => 'GET',
...
/**
 * Commit 1.11.b shared CSRF gate for state-mutating gTemplate REST
 * endpoints. Verifies the `X-WP-Nonce` header against WP's canonical
 * `wp_rest` action — that nonce is auto-generated by `wp_localize_script`
 * for the `wpApiSettings` global and sent by `wp.apiFetch` and any
 * other code that uses WordPress's standard REST plumbing. Returns
 * `WP_Error(403)` on miss so the REST layer emits a consistent
 * `gtemplate_csrf_invalid` response.
 *
 * Reused by /cookie-consent POST + /language POST.
 */
function gtemplate_rest_verify_nonce(\WP_REST_Request $request)
{
    $nonce = $request->get_header('x_wp_nonce');
    if (!$nonce || !wp_verify_nonce((string) $nonce, 'wp_rest')) {
        return new \WP_Error(
            'gtemplate_csrf_invalid',
            'CSRF nonce missing or invalid. Refresh the page and try again.',
            ['status' => 403]
        );
    }
    return true;
