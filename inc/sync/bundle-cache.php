...
/**
 * gTemplate bundle-cache — face mapping + post-bundle pre-render to ValKey
 *
 * Extracted from inc/gnode-content-sync.php in Commit 1.10.c.
 * Owns the "ValKey-backed pre-rendered bundle" subsystem that the gNode
 * daemon's bundle builder consumes:
 *   - sync face mapping configuration to ValKey (gtemplate_sync_face_mapping_to_valkey)
 *   - build per-face data + render templates (gtemplate_build_face_data,
 *     render_page_content, render_posts_list, pre_render_template, ...)
 *   - per-post bundle pre-render + cache (gtemplate_cache_post_bundle,
 *     cache_all_post_bundles, invalidate_post_bundle_cache,
 *     recache_post_bundle_handler)
 *   - generate_post_bundle (canonical pre-render path)
 *   - bundle-rebuild trigger publishing to gNode (gtemplate_trigger_bundle_rebuild)
 *   - WordPress hook wiring: save_post / before_delete_post / customize_save_after
 *     → invalidate or re-cache bundles
 *
 * @package gTemplate
 * @since 1.1.0
 */
 ...
 *
 * Stores the full face configuration including:
...
function gtemplate_sync_face_mapping_to_valkey(): bool {
    ...
    try {
        ...
        if (!$gNodeClient) {
            gtemplate_track_error("gTemplate: Cannot sync face mapping - gNode client not available");
            return false;
        }
        ...
        if ($result !== false && $result !== null) {
            gtemplate_track_error("gTemplate: Face mapping synced to ValKey ({$key}) - " . strlen($json) . " bytes");

            // Invalidate keybased-client bundle cache (CB-D1.13 mirror:
            // folded in from the now-removed gtemplate_sync_face_configs_to_valkey
            // hook so this single canonical hook performs both invalidation
            // paths). gCube applied the same consolidation in commit 2.0.
            if (function_exists('gtemplate_invalidate_bundle')) {
        ...
        gtemplate_track_error("gTemplate: Failed to store face mapping in ValKey (result: " . var_export($result, true) . ")");
        return false;

    } catch (\Throwable $e) {
        gtemplate_track_error("gTemplate: Face mapping sync error: " . $e->getMessage());
        return false;
    }
}
...
function gtemplate_pre_render_template(string $template_name, array $variables): string {
    ...
    if ($gNode) {
        try {
            ...
            gtemplate_track_error("gTemplate: Daemon template render failed, using PHP fallback: " . $e->getMessage());
        }
    }
    ...
    // Handle `| safe` filter — DON'T trust the contract.
    //
    // Commit 1.11.a: pre-fix substituted `$value` raw on the
    // assumption that any template author writing `{{ x | safe }}`
    // had already sanitized $value. That contract is unenforceable
    // from outside the template; WP post content with unfiltered_html
    // capability flowed straight through, turning `| safe` into a
    // template-author-induced XSS primitive.
    //
    // Post-fix: route `$value` through `wp_kses_post` even on the
    // `safe` branch. wp_kses_post accepts the WP-canonical "safe HTML
    // for post content" subset (including links, lists, formatting,
    // tables, images) and strips scripts / iframes / event handlers /
    // javascript: URLs / unknown tags. Templates that legitimately
    // need richer HTML can extend the kses allowlist via the
    // `wp_kses_allowed_html('post')` filter.
    foreach ($variables as $key => $value) {
        if (is_string($value)) {
            $html = str_replace("{{ {$key} | safe }}", wp_kses_post($value), $html);
        }
    }
...
function gtemplate_cache_post_bundle(string $slug, ?array $bundle_config = null): bool {
    try {
        ...
        if ($bundle_config === null) {
            ...
            if (!$bundle_config) {
                gtemplate_track_error("gTemplate: Bundle '{$slug}' not found");
                return false;
            }
        }
        ...
        if ($result !== false && $result !== null) {
            gtemplate_track_error("gTemplate: Cached post bundle '{$slug}' ({$cache_data['post_count']} posts) - " . strlen($json) . " bytes");
            return true;
        }

        gtemplate_track_error("gTemplate: Failed to cache post bundle '{$slug}'");
        return false;

    } catch (\Throwable $e) {
        gtemplate_track_error("gTemplate: Post bundle cache error: " . $e->getMessage());
        return false;
    }
}
...
function gtemplate_cache_all_post_bundles(): array {
    ...
    gtemplate_track_error("gTemplate: Cached {$results['cached']} post bundles, {$results['failed']} failed");
    return $results;
}
    ...
    foreach ($bundles as $slug => $config) {
        if (in_array($post_id, $config['post_ids'], true)) {
            ...
            try {
                ...
                gtemplate_track_error("gTemplate: Failed to invalidate bundle '{$slug}': " . $e->getMessage());
            }
        }
    }

    if (!empty($invalidated)) {
        gtemplate_track_error("gTemplate: Invalidated post bundles for post {$post_id}: " . implode(', ', $invalidated));
    }
}
...
// ============================================================================
// BLOG BUNDLE — every published post, one ValKey read
// A "blog" face renders all posts client-side from a single bundle, so post
// switching costs no network round-trip. Key: {site}:post_bundle:__blog
// ============================================================================

/**
 * The whole blog as one bundle: try ValKey, else build from WP and warm it.
 *
 * Each entry carries the fully rendered content, so a blog face can embed the
 * lot and swap posts in place. Cache is invalidated on any post save/delete
 * (see the __blog branch in the save/delete hooks).
 *
 * @param int $limit Max posts (newest first).
 * @return array{posts: array<int, array<string, mixed>>, cached: bool}
 */
function gtemplate_get_blog_bundle(int $limit = 30): array {
    $site_id = function_exists('\gTemplate\get_site_id_from_domain')
        ? \gTemplate\get_site_id_from_domain() : 'default';
    $key = "{$site_id}:post_bundle:__blog";

    // 1. Try ValKey.
    try {
        $gNodeClient = gtemplate_gnode();
        if ($gNodeClient) {
            $cached = $gNodeClient->fcall('GNODE_CACHE_GET', [], [$key, $site_id]);
            if ($cached && is_string($cached)) {
                $data = json_decode($cached, true);
                if (!empty($data['posts']) && is_array($data['posts'])) {
                    return ['posts' => $data['posts'], 'cached' => true];
                }
            }
        }
    } catch (\Throwable $e) {
        gtemplate_track_error('gTemplate: blog bundle cache read error: ' . $e->getMessage());
    }

    // 2. Build from WordPress (source of truth).
    $posts = get_posts([
        'post_type'      => 'post',
        'post_status'    => 'publish',
    ...
    // 3. Warm ValKey (best-effort).
    try {
        if (!empty($posts_data) && isset($gNodeClient) && $gNodeClient) {
            $payload = json_encode(
                ['posts' => $posts_data, 'cached_at' => time()],
                JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE
            );
            $gNodeClient->fcall('GNODE_CACHE_SET', [], [$key, $payload, 3600, $site_id]);
        }
    } catch (\Throwable $e) {
        gtemplate_track_error('gTemplate: blog bundle cache warm error: ' . $e->getMessage());
    }

    return ['posts' => $posts_data, 'cached' => false];
}

/**
 * Drop the blog bundle so the next read rebuilds it.
 */
function gtemplate_invalidate_blog_bundle(int $post_id = 0): void {
    try {
        $gNodeClient = gtemplate_gnode();
        if (!$gNodeClient) {
            return;
...
add_action('save_post_post', 'gtemplate_invalidate_blog_bundle', 20, 1);
add_action('before_delete_post', 'gtemplate_invalidate_blog_bundle', 20, 1);
...
function gtemplate_trigger_bundle_rebuild(string $site_id): void {
    try {
        ...
        gtemplate_track_error("gTemplate: Bundle rebuild triggered for {$site_id}");

    } catch (\Throwable $e) {
        gtemplate_track_error("gTemplate: Failed to trigger bundle rebuild: " . $e->getMessage());
    }
}
...
function gtemplate_get_template_js_content(string $template_name): ?string {
    ...
    if (!file_exists($js_path)) {
        gtemplate_track_error("gTemplate: Template JS file not found: {$js_path}");
        return null;
    }

    $content = file_get_contents($js_path);
    if ($content === false) {
        gtemplate_track_error("gTemplate: Failed to read template JS file: {$js_path}");
        return null;
    }
...
function gtemplate_generate_post_bundle(int $post_id): ?array {
    $post = get_post($post_id);
    if (!$post) {
        gtemplate_track_error("[gTemplate Bundle] Post {$post_id} not found");
        return null;
    }
    ...
    if ($keybased) {
        try {
            ...
            gtemplate_track_error("[gTemplate Bundle] KeyBased render failed for {$post_id}: " . $e->getMessage());
        }
    }
    ...
    if (!$html) {
        gtemplate_track_error("[gTemplate Bundle] No HTML generated for post {$post_id}");
        return null;
    }
    ...
    if ($keybased) {
        try {
            ...
            gtemplate_track_error("[gTemplate Bundle] ValKey store failed for {$post_id}: " . $e->getMessage());
        }
    }
    ...
    if (!$stored) {
        ...
        gtemplate_track_error("[gTemplate Bundle] Stored post {$post_id} as transient fallback");
    }
    ...
    gtemplate_track_error("[gTemplate Bundle] Generated bundle for post {$post_id}: {$valkey_key} (" . size_format($size) . ")");
